...
# 5. Tanítási ciklus (mixed precision támogatással)
def train_model(model, train_loader, test_loader, criterion, optimizer, scaler, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            optimizer.zero_grad()
            scaler.scale(loss).backward()  # Loss skálázása
...
def evaluate_model_on_loader(model, data_loader, device, dataset_name="Dataset"):
    model.eval()
    y_true, y_pred = [], []
    with torch.no_grad():
    ...
    print(f"\nTotal Correct Predictions for {dataset_name}: {total_correct}")
    print(f"Total Incorrect Predictions for {dataset_name}: {total_incorrect}")

# --- Fő futtatás ---
if __name__ == "__main__":
    file_path = "ma20_14_ai_chk2.txt"
    ...
    # Kiértékelés a tanító adatokon
    evaluate_model_on_loader(model, train_loader, device, dataset_name="Training Data")

    # Kiértékelés a teszt adatokon
    evaluate_model_on_loader(model, test_loader, device, dataset_name="Test Data")